// copyright 2017: Kai Chen, Hsieh, NJIT ID: 31396044
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <string.h>
...
void create_leaf(int value, int freq, struct node *leaf) {
	leaf->value = value;
	leaf->freq = freq;
	leaf->is_leaf = true;
	leaf->left = NULL;
...
void create_internal_node(struct node *new_node, struct node *left_child, struct node *right_child) {
...
int insert_node(struct node *new_node, struct node *heap[256], int heap_size) {
	if (heap_size >= 256) {
...
struct node* extract_heap(struct node *heap[256], int last) {
...
void adjust_heap(struct node *heap[256], int last) {
	int i = last;
	struct node *temp;
...
struct node* build_huffman_tree(struct node *heap[256], int heap_size) {
	struct node* left;
	struct node* right;
	...
	while(heap_size != 1) {
		left = extract_heap(heap, --heap_size);
		printf("Pop freq: %d ", left->freq);
		if (left->is_leaf) {
			printf("value: %d", left->value);
		}
		printf("\n");
		right = extract_heap(heap, --heap_size);
		...
		create_internal_node(new_node, left, right);
		heap_size = insert_node(new_node, heap, heap_size);
...
void dfs(struct node *curr_node, struct code code[256], int bitmap, int len) {
	...
	if (curr_node->is_leaf) {
		...
		dfs(curr_node->left, code, bitmap + 1, len + 1);
		dfs(curr_node->right, code, bitmap, len + 1);
	}
}

void encode(char *buff, long size, char *outfile, long outfile_size, struct code code_list[256]) {
...
void decode(char *in_buff, long infile_size, char *outfile, long outfile_size, struct node *root) {
	int i, index, loc, value, len, remain;
	int bit_offset;
	size_t ret;
		...
		while(bit_offset >= 0) {
			...
			if(curr_node->is_leaf) {
				...
				if(index == outfile_size) {
					printf("finished decoding all symbols \n");
...
int main(int argc, char **argv) {
	...
	for(i = 0; i < 256; i++) {
		...
		new_node = (struct node*)malloc(sizeof(struct node));
		create_leaf(i, count[i], new_node);
		heap_size = insert_node(new_node, node_list, heap_size);
		adjust_heap(node_list, heap_size - 1);
	}
	display_heap(node_list, heap_size);

	root = build_huffman_tree(node_list, heap_size);
	printf("Heap size: %d \n", heap_size);
	printf("Root freq: %d \n", root->freq);
	printf("root->left->freq: %d \n", root->left->freq);
	printf("root->right->freq: %d \n", root->right->freq);

	/*printf("Extracting heap nodes: \n");
	for(i = 0; i < 256; i++) {
		new_node = extract_heap(node_list, heap_size - 1);
		heap_size--;
		printf("%d ", new_node->freq);
	}*/

	printf("Creating Huffman codes \n");
	// create Huffman codes
	dfs(root, code_list, -1, 0);
	
	printf("Decoding file \n");
	decode(buff, size, outfile, outfile_size, root);
